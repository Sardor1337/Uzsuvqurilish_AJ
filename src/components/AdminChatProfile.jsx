...
const AdminChatProfile = ({ selectedMchj }) => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/GetMessagesBetweenAdminAndMCHJ/mchj/${mchjId}/`
      );
  ...
  useEffect(() => {
    // Har sekundda so‘rov yuborish uchun interval o‘rnatamiz
  ...
  }, [mchjId]); // mchjId o‘zgarganda useEffect qayta ishlaydi
  ...
  const sendMessage = async () => {
    if (inputText.trim() === "" || loading || !mchjId) return;
    setLoading(true);

    try {
      const response = await fetch(
        `${API_URL}/api/messages/mchj/send/${mchjId}/`,
        {
          method: "POST",
  ...
  const handleContextMenu = (e, msg) => {
    e.preventDefault(); // Brauzer standart menyusini bloklash
    if (msg.sender === 1) return; // Faqat admin xabarlariga menyuni chiqarish
  ...
  const handleUpdate = async (messageId, role) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/messages/${messageId}/${role}/`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (messageId, role) => {
    try {
      const response = await fetch(
        `${API_URL}/api/messages/${messageId}/${role}/`,
        { method: "DELETE" }
    ...
    <div className="w-3/4 flex flex-col">
      {selectedMchj ? (
        <div className="flex flex-col flex-1 p-6 bg-white">
          <h2 className="text-lg font-bold pb-3 border-b-2 border-gray-100 mb-4">
            {selectedMchj.name}
          </h2>
          ...
          {contextMenu && (
            <div
              ...
              style={{
                top: contextMenu.y,
                left:
                  contextMenu.x + 150 > window.innerWidth
                    ? contextMenu.x - 120
                    : contextMenu.x,
              }}
            >
...
export default AdminChatProfile;
